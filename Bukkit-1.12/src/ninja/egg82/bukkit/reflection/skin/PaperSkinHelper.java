...
public class PaperSkinHelper implements ISkinHelper {
	//vars
	...
	public ItemStack getSkull(UUID playerUuid, int amount, boolean expensive) {
		...
		PlayerProfile player = Bukkit.createProfile(playerUuid);
		String name = player.complete(false) ? player.getName() : playerUuid.toString();
		...
		skull.setPlayerProfile(getProfile(playerUuid, name, expensive));
	...
	public Skin getSkin(UUID playerUuid, boolean expensive) {
		...
		if (FileUtil.pathExists(skinFile) && FileUtil.pathIsFile(skinFile)) {
			...
			if (retVal != null) {
				// Add current cached skin to memory (at least until new skin is fetched form network)
		...
		// If not using an expensive lookup, return default skin. Otherwise fetch from network
		return (expensive) ? getSkinExpensive(playerUuid) : getDefaultSkin();
	}
	
	//private
	private PlayerProfile getProfile(UUID playerUuid, String playerName, boolean expensive) {
		PlayerProfile retVal = Bukkit.createProfile(playerUuid, playerName);
		retVal.setProperty(new ProfileProperty("textures", getSkin(playerUuid, expensive).data.texture.value));
		return retVal;
	}
