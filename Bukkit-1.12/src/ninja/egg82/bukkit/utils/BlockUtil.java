...
public final class BlockUtil {
    ...
    public static Location getNearestAirBlock(Location l, int maxRadius) {
        ...
        if (!Bukkit.isPrimaryThread() && !l.getWorld().isChunkLoaded(l.getBlockX() >> 4, l.getBlockZ() >> 4)) {
            // If we're not on the server thread and the chunk isn't loaded, don't even
            // bother trying
            return LocationUtil.toBlockLocation(l);
        }

        // We don't want to modify the original Location, and we want nice block
        // locations
    ...
    public static Location getNearestSolidBlock(Location l, int maxRadius) {
        if (l == null) {
            throw new IllegalArgumentException("l cannot be null.");
        }
        if (maxRadius < 1) {
        ...
        if (l.getBlock().getType().isSolid()) {
            // Got lucky, the current block is solid
            return l;
        }
        ...
        for (int x = maxRadius * -1; x <= maxRadius; x++) {
            ...
            for (int z = maxRadius * -1; z <= maxRadius; z++) {
                ...
                for (int y = maxRadius * -1; y <= maxRadius; y++) {
                    // Need a new loc each time so we don't screw up the math
                    Location l2 = l.clone().add(x, y, z);
                    if (l2.getBlock().getType().isSolid() && (closest == null || l2.distanceSquared(l) < closest.distanceSquared(l))) {
                        // Found a solid block!
                        closest = l2;
                    }
                }
    ...
    public static Location getLowestAirBlock(Location l) {
        ...
        if (type == Material.AIR) {
            ...
            // We don't care if 0 is the "lowest" air block because technically that's
            // correct
            // If the block isn't air, add 1 to it
            return (type == Material.AIR) ? l : l.add(0.0d, 1.0d, 0.0d);
        }
        ...
        // We don't care if maxHeight is the "lowest" air block because technically
        // that's correct
        return l;
    }
    ...
    public static Location getHighestSolidBlock(Location l) {
        ...
        if (!type.isSolid()) {
            ...
            // We don't care if 0 is the "highest" solid block because technically that's
            // correct
            return l;
        }

        // The block is solid, so we need to scan upwards to find the first non-solid
        // block
        while (l.getY() < l.getWorld().getMaxHeight() && type.isSolid()) {
            l.add(0.0d, 1.0d, 0.0d);
            type = l.getBlock().getType();
        }
        // We don't care if maxHeight is the "highest" solid block because technically
        // that's correct
        // If the block isn't solid, subtract 1 from it
        return (type.isSolid()) ? l : l.add(0.0d, -1.0d, 0.0d);
    }
    ...
    public static BlockData getBlock(Location location, BlockState blockState) {
        ...
        return new BlockData(LocationUtil.toBlockLocation(location), blockState.getType(), blockState.getRawData(),
            ServiceLocator.getService(ISerializationHelper.class).toCompressedBytes(blockState));
    }
    ...
    public static void setBlocks(List<BlockData> blocks, Location center, int xRadius, int yRadius, int zRadius, boolean updatePhysics) {
        ...
        if (blocks.size() != (xRadius * 2 + 1) * (yRadius * 2 + 1) * (zRadius * 2 + 1)) {
            throw new RuntimeException(
                "blocks is not the correct length for the radii specified. Expected " + ((xRadius * 2 + 1) * (yRadius * 2 + 1) * (zRadius * 2 + 1)) + ", got " + blocks.size() + ".");
        }
        if (center == null) {
            throw new IllegalArgumentException("center cannot be null.");
        }
